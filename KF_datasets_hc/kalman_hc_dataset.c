...
void kalman_filter(float* initial, float* P_flat, float* A, float* Q, float* R, float* H, 
                   float* measurements, float* real_out, float* prediction, int iter) {
...
int check_error(float measurements[], float temp_var[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        float diff = measurements[i] - temp_var[i];
        mse += pow(diff, 2);
    }
    mse /= size;
