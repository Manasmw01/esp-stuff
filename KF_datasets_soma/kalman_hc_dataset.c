...
int check_error(double measurements[], double temp_var[], int size);
...
void kalman_filter(double* initial, double* P_flat, double* A, double* Q, double* R, double* H, 
    ...
    for (int i = 0; i < STATE_SIZE; i++) 
    {
        temp_var[i] = prediction[STATE_SIZE*(iter+1) + i];
    }
    printf("Expected\n");
    print_vector(temp_var, STATE_SIZE);
    tot_errors += check_error(xp, temp_var, STATE_SIZE);
    ...
    // Compute HPHT + R
    matrix_add(HPHT, Q, HPHT, MEAS_SIZE, MEAS_SIZE);  // (H * Pp) * H^T + R 
    // printf("HPHT\n");
    // print_matrix(HPHT, MEAS_SIZE, MEAS_SIZE);
    ...
    gauss_inverse(HPHT, HPHT_inv, MEAS_SIZE); // inv((H * Pp) * H^T + R)
    ...
    for (int i = 0; i < MEAS_SIZE; i++) 
    {
        temp_var[i] = measurements[MEAS_SIZE*(iter) + i];
    }
    // printf("Measurements\n");
    // print_vector(temp_var, MEAS_SIZE);
    ...
    // printf("\n\nupdated state vector(2nd measurement in the prediction_array_soma.h file): ");
...
int check_error(double measurements[], double temp_var[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        double diff = measurements[i] - temp_var[i];
        mse += pow(diff, 2);
    }
    mse /= size;

    if (mse > ERROR_THRESHOLD) {
        printf("===========================MSE exceeded threshold=====================: %f > %f\n", mse, ERROR_THRESHOLD);
        return 1;
    } else {
        return 0;
    }
