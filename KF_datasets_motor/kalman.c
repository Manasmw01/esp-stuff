...
void print_vector(double* vector, int size);
    ...
    double HtA[MEAS_SIZE * STATE_SIZE];
    double Y[MEAS_SIZE];
    double Y_tmp[MEAS_SIZE];
...
void kalman_filter(double* initial, double* P_flat, double* A, double* Q, double* R, double* H, 
    ...
    // xp = A * x
    // double xp[STATE_SIZE];
    matrix_multiply(H, A, HtA, MEAS_SIZE, STATE_SIZE, STATE_SIZE); // xp = A*x2

    matrix_multiply(HtA, x, Y_tmp, MEAS_SIZE, STATE_SIZE, 1); // xp = A*x2
    ...
    matrix_subtract(temp_meas, Y_tmp, Y, MEAS_SIZE); // P3 = Pp - (K * H * Pp)

    printf("intermediate state vector\n");
    print_vector(Y, STATE_SIZE);
